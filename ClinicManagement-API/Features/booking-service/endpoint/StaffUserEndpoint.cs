...
public static class StaffUserEndpoint
{
    ...
    {
        var app = route.MapGroup("/api/admin/staff-user")
            .WithTags("StaffUsers Management")
        ...
        app.MapGet("/", async (ClinicDbContext context) => Results.Ok(new ApiResponse<List<StaffUserDto>>(
            ...
            await context.StaffUsers.AsNoTracking().Select(s =>
                    new StaffUserDto(s.UserId, s.ClinicId, s.Username, s.FullName, s.Role, s.IsActive, s.Clinic))
                .ToListAsync())));
    }
}