...
public static class ClinicEndpoint
{
    ...
    {
        var app = route.MapGroup("/api/admin/clinic")
            .WithTags("Clinics Management")
        ...
        app.MapGet("/", async (ClinicDbContext context) => Results.Ok(new ApiResponse<List<ClinicDto>>(
            ...
            await context.Clinics.AsNoTracking()
                .Select(k => new ClinicDto(k.ClinicId, k.Code, k.Name, k.TimeZone, k.Phone, k.Email))
                .ToListAsync()
        )));
    }
}
